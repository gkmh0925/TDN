...
public class ExcelReadCalc_LOGIALL {
	public static HashMap<String, List<Map<String, Object>>> read(ExcelReadOption excelReadOption,Map<String, Object> requestParamMap) {
		...
		for (int i = 0; i < sheetCnt; i++) {
			...
			if (wb.getSheetName(i).equals("생각대로 정산리스트")) {
				rowIndex = excelReadOption.getStartRow() - 1;
			}

			for (rowIndex = excelReadOption.getStartRow()-1; rowIndex <= (excelReadOption.getStartRow()+numOfRows-1); rowIndex++) {
				...
				if (row != null) {
					...
					for (int cellIndex = 0; cellIndex < numOfCells; cellIndex++) {
						...
						if (cellName.equals("A")) {
							...
							data = StringUtils.leftPad(data, 5, "0"); // 왼쪽에 0으로 채우기
						...
						if (cellName.equals("C")) {
							//SimpleDateFormat dateCell = new SimpleDateFormat("yyyy-MM-dd");
							//String data = dateCell.format(cell.getDateCellValue()).replaceAll("[^0-9]", "").trim();
							String data = ExcelCellRef.getValue(cell).replaceAll("[^0-9]", "").trim();
