...
$(document).ready(function() {
	...
	grid = new dhx.Grid("grid", {
		columns: [
			...
			{ id: "prd_nm", header: [{ text: "상품명", align: "center" }] },
//			{ id: "ctgr_type_nm", header: [{ text: "매장카테고리명", align: "center" }] },
			{ id: "prd_state", header: [{ text: "상품상태", align: "center" }] },
			{ id: "std_amt", header: [{ text: "판매단가", align: "center" }] },
			{ id: "seq", header: [{ text: "정렬순서", align: "center" }] },
			{ id: "reg_user_nm", header: [{ text: "등록사용자", align: "center" }] },
			{
			...
			{ id: "mdf_user_nm", header: [{ text: "수정사용자", align: "center" }] },
			{
...
function search_go(num) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			let obLength = Object.keys(data.deliveryList).length;
			if (obLength == 0) {
				swal({
					text: "조회된 결과가 없습니다.",
					icon: "warning"
...
//				$("#excel_download").attr("disabled", true);
				...
				grid.data.parse(data.deliveryList);
...
function eventHandler(event, arguments) {
	...
	console.log("배달상품 더블클릭!");
	
	const chCd = arguments[0].ch_cd;
	const prdCd = arguments[0].prd_cd;

	let form = document.createElement("form");
	form.target = "_blank";
	form.method = "POST";
	form.action = "deliveryProductDetail";
	form.style.display = "none";

	let InputCpnCd = document.createElement("input");
	InputCpnCd.type = "text";
	InputCpnCd.name = "chCd";
	InputCpnCd.value = chCd;

	let InputPrdCd = document.createElement("input");
	InputPrdCd.type = "text";
	InputPrdCd.name = "prdCd";
	InputPrdCd.value = prdCd;

	form.appendChild(InputCpnCd);
	form.appendChild(InputPrdCd);
