...
$(document).ready(function() {
	...
	grid = new dhx.Grid("grid", {
	    columns: [
	        ...
	        { id: "menu_CD", header: [{ text: "메뉴코드", align: "center" }] },
	        { id: "menu_NM", header: [{ text: "메뉴명", align: "center" }] },
	        { id: "price", header: [{ text: "단가(원)", align: "center" }], type: "number", format: "#,#" },
	        { id: "menu_STATE", header: [{ text: "메뉴상태", align: "center" }] },
	        { id: "menu_STATE_MDF_DATE", header: [{ text: "상태수정일시", align: "center" }],
	       		template: function (text, row, col) {
					return unixTimeFormat(text);
            	}
            },
	        { id: "ctgr_NM", header: [{ text: "매장 내 카테고리명(FC)", align: "center" }] },
	        { id: "reg_USER_NM", header: [{ text: "등록자명", align: "center" }] },
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (obLength == 0) {
				...
				grid.data.parse(data.productList);
...
function eventHandler(event, arguments) {
	const plucd = arguments[0].menu_CD;
	const scd = $("#scd").val();
